...
def main():
    ...
    # object = 'example'
    pic_dir = '/data/pics'
    '''mfc_dir = '/data/webdav/ctbrec/videos'
    mfc_members = get_members(mfc_dir)
    for member in mfc_members:
        vid_dirs = get_members(mfc_dir + '/' + member)
        make_dir(pic_dir + '/mfc/' + member)
        pic_dirs = find_in_dir(pic_dir + '/mfc/' + member, 'jpg')

        for i in range(len(pic_dirs)):
            pic_dirs[i] = pic_dirs[i].split('.')[0]
        pic_to_create = [item for item in vid_dirs if item not in pic_dirs]
        print(pic_to_create)
        # print(' pic_dirs = '+ str(pic_dirs))
        for vid_dir in pic_to_create:
            cut_ctbrec(mfc_dir + '/' + member + '/' + vid_dir, pic_dir + '/mfc')
        # find what's only in pic_dir and del the pic
        pic_to_del = [item for item in pic_dirs if item not in vid_dirs]
        print('to del ' + str(pic_to_del))
        del_pics(pic_to_del, pic_dir + '/mfc/' + member, 'jpg')
        # print(sub_pic_name)
    member_to_del = [item for item in get_members(pic_dir + '/mfc') if item not in mfc_members]
    print('member_to_del = ' + str(member_to_del))
    for member in member_to_del:
        del_dir(pic_dir + '/mfc/' + member)
        # del_pics(pic_to_del, sub_pic_dir, pic_format)'''
    ctr_dir = '/root/recordurbate/videos'
    members = get_members(ctr_dir)
    print('ctb_members = '+str(members))

    for member in members:
        do_cut(ctr_dir + '/' + member)
